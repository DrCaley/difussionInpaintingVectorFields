#!/usr/bin/env python3
"""Generate individual quiver-plot figures: one image per method.

Outputs 4 standalone PNGs into paper/figures/:
  fig_gt.png, fig_gp.png, fig_vcnn.png, fig_composite.png

Usage:
    PYTHONPATH=. python scripts/generate_single_panels.py [--reveal-pct 5.0] [--val-idx 460]
"""
...
def load_ddpm(dev):
    ...
    ddpm.load_state_dict(torch.load(DDPM_WEIGHT, map_location="cpu", weights_only=False))
    ddpm.to(dev); ddpm.eval()
    return ddpm


def load_gp_cnn(dev):
    ck = torch.load(GP_CNN_CKPT, map_location="cpu", weights_only=False)
    cfg = ck["model_config"]
...
def load_vcnn(dev):
    ck = torch.load(VCNN_CKPT, map_location="cpu", weights_only=False)
...
# ── single-panel plotter (uses project's plot_vector_field) ──────────

def plot_single(vel_np, ocean_mask_np, title, out_path, obs_mask=None):
    """Produce one quiver image using the project's standard style."""
    ...
    # Build missing_mask overlay if we have obs_mask
    miss = None
...
def main():
    ...
    parser.add_argument("--t-val", type=int, default=75)
    ...
    # data – shape after load: (N, 2, 44, 94)
    val = load_data()
    gt_full = val[args.val_idx]                       # (2, 44, 94)
    gt_crop = gt_full.numpy()                         # already ocean-sized
    ...
    print(f"Sample val[{args.val_idx}], {args.reveal_pct}% coverage "
          f"({int(obs_mask.sum())} pts)")

    # ── GT ─────────────────────────────────────────────────────────
    print("Plotting GT …")
    plot_single(gt_crop, ocean_mask, "Ground Truth", OUT_DIR/"fig_gt.png")

    # ── GP ─────────────────────────────────────────────────────────
    print("Computing GP …")
    gp_mean, gp_var = compute_gp(vel_phys, obs_mask, ocean_mask)
    plot_single(gp_mean, ocean_mask, "Gaussian Process", OUT_DIR/"fig_gp.png",
                obs_mask=obs_mask)
    ...
    plot_single(vcnn_pred, ocean_mask, "Voronoi-CNN", OUT_DIR/"fig_vcnn.png")
    ...
    plot_single(comp_np, ocean_mask, "GP-CNN / DDPM Composite",
                OUT_DIR/"fig_composite.png")

    print("\nDone — 4 figures in", OUT_DIR)
