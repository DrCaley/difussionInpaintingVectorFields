...
class ModelInpainter:
    ...
    def plot_mse_vs_mask_percentage(self):
        ...
        plt.title(f"MSE vs. Mask Coverage Percentage (Model: GP")
        plt.xlabel("Percentage of Masked Pixels")
        plt.ylabel("MSE")
        plt.grid(True)
        plt.tight_layout()
        plot_path = os.path.join(self.results_path, f"mse_vs_mask_percentage_GP.png")
    ...
    def _inpaint_testing(self, mask_generator: MaskGenerator, image_counter: int, file: csv.writer):
        ...
        with tqdm(total=min(len(loader.dataset), num_images_to_process),
            ...
            for step, batch in enumerate(loader):
                ...
                input_image_original = self.dd.get_standardizer().unstandardize(torch.squeeze(input_image, 0)).to(
                    device)
                ...
                with torch.no_grad():
                    for resample in self.resamples:
                        for i in tqdm(range(n_samples), leave=False, desc="Samples", colour="#006666"):
                            final_image_ddpm = torch.zeros_like(input_image)

                            standardizer = self.dd.get_standardizer()
                            final_image_ddpm = torch.unsqueeze(
                                standardizer.unstandardize(torch.squeeze(final_image_ddpm, 0)).to(device), 0)

                            gp_field = gp_fill(input_image_original, mask)
                            final_image_ddpm = gp_field.clone()
                            ...
                            # MSE + Coverage
                            mse_ddpm = calculate_mse(input_image_original_cropped, final_image_ddpm_cropped,
                                                     mask_cropped)
                            ...
                            writer.writerow(
                                [self.model_name, image_counter, mask_generator, num_lines, resample, mse_ddpm.item(),
                                 mask_percentage])
